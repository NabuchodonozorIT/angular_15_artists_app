...
export class ListComponent implements OnInit {
  items: any[] = [];
  add: (...args: any) => void;
  logger: (...args: any) => (...args: any) => void;

  constructor() {
    this.logger = () => {
      this.items = [];
      return (...args) => this.add(...args);
    };
    this.add = (...args: any) => {
      ...
      if (args.length === 1) {
        item = {
          label: '',
          item: CircularJSON.stringify(args[0])
        };
      } else {
        item = {
          label: args[0],
          item: CircularJSON.stringify(args[1])
        };
      }
          ...
          current.length ?
            (
              retrieve.hasOwnProperty(current) ?
                ...
                retrieve[<any>current] = retrieveFromPath(
